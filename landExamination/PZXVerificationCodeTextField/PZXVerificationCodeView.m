...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.deselectColor = [UIColor blueColor];   //默认边框颜色
        self.alpha = 0.7;
        [self setView]; //绘制界面
...
//        tf.layer.cornerRadius = 6;
        ...
        [tf addTarget:nil action:@selector(tfEditingChanged:) forControlEvents:UIControlEventEditingChanged];
...
- (void)tfEditingChanged:(id)sender{
    
    PZXVerificationTextField *textField = sender;
    textField.layer.borderColor = self.selectedColor.CGColor;
    [self getVertificationCode];
}
